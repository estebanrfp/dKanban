...
export default async function mountKanban ({ db, mount, marked, onLogout, state = {} }) {
  ...
  // Identify current user via Security Manager (documented API)
  const getActive = typeof db?.sm?.getActiveEthAddress === 'function' ? db.sm.getActiveEthAddress.bind(db.sm) : null
  const activeAddress = getActive ? getActive() : undefined
  if (!activeAddress) {
    console.warn('No active address found; redirecting to signing screen.')
    if (typeof onLogout === 'function') return onLogout()
    return
  }
  const boardId = `kanban-board-${ activeAddress }`

  // Show abbreviated address if provided or derive from activeAddress
  const abbr = addr => addr ? `${ addr.slice(0, 6) }…${ addr.slice(-4) }` : ''
  const userAbbr = state.abbrAddr || abbr(activeAddress)
  const abbrEl = document.getElementById('user-abbr')
  if (abbrEl) abbrEl.textContent = userAbbr

  // Render tablero default inmediatamente para evitar pantalla vacía
  buildRetrievedProject(json)
  updateProgress()

  // reactive board listener
  // Ensure per-user default board exists, then subscribe to only this board
  try {
    const existing = await (db.get ? db.get(boardId) : null)
    if (!(existing && (existing.value || existing.result?.value))) {
      // persist default for this user
      const initial = { type: 'board', id: boardId, owner: activeAddress, ...json }
      try { await db.put(initial, boardId) } catch (_) {}
    }
  } catch (_) {
    // Fallback: create anyway (safe because id is per-user)
    const initial = { type: 'board', id: boardId, owner: activeAddress, ...json }
    try { await db.put(initial, boardId) } catch (_) {}
  }
  ...
  async function updateProjectData () {
    ...
    // persist full board on every structural change
    try {
      await db.put({ type: 'board', id: boardId, owner: activeAddress, ...update }, boardId)
    } catch (e) {
  ...
  async function logout () {
    try {
      ...
      if (db.sm) await db.sm.clearSecurity()
      // Remove mnemonic auto-login markers so next load requires explicit auth
      try {
        const last = localStorage.getItem('webauthn-last')
        if (last) {
          localStorage.removeItem(`webauthn-mnemonic-${last}`)
        }
        localStorage.removeItem('webauthn-last')
      } catch (_) {}
    } finally {
  ...
  // expose cleanup (optional future usage)
  return () => {
