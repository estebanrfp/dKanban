...
export default async function mountKanban ({ db, mount, marked, onLogout }) {
  ...
  const { unsubscribe } = await db.map({ query: { type: 'board' } }, ({ value }) => {
    // Si llega un valor válido con estructura esperada, lo usamos; si no, mantenemos existente
  ...
  async function updateProjectData () {
    ...
    try {
      await db.put({ type: 'board', ...update }, 'kanban-board')
    } catch (e) {
