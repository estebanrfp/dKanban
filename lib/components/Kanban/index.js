...
export default async function mountKanban ({ db, mount, marked, toast, onLogout, state = {} }) {
  ...
  async function updateProjectData () {
    ...
    for (let i = 0; i < allCols.length; i++) {
      ...
      for (let k = 0; k < cards.length; k++) {
        ...
        const card = new Card(cardNumber, cardText)
        cardData.push(card)
      }
  ...
  function buildRetrievedProject (data) {
    ...
    const projectHTML = `${ projectData.data.map((col, i) =>
      `<div id="stage-${ col.colNumber }" class="col ${ col.tracking }-gradient ${ col.tracking }-border" data-track="${ col.tracking }" draggable="true">
        ...
        `<div id="card-${ card.cardNumber }" class="card" draggable="true" data-card-track="${ col.tracking }" data-card-md="${ card.cardText }">
  ...
  function addCard (column) {
    ...
    const cardTemplate = `
      <div id="card-${ newCardNo }" class="card" draggable="true" data-card-track="${ colTracking }" data-card-md="${ newNoteText }">
