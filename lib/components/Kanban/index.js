...
// but route through a mutable ref so drops always hit the CURRENT mount's
// persistence (a fixed closure would keep stale board/permissions forever).
let dragWired = false
let dragUpdate = () => {}
...
export default async function mountKanban ({ db, mount, marked, toast, onLogout, state = {} }) {
  ...
  // Integrity guard: a truncated/stale-cached template without the header
  // areas would leave a broken shell — fall back to the inline one.
  if (!document.getElementById('columnsContainer') || !document.getElementById('user-info')) {
  ...
  // The address is a click-to-copy button so a collaborator can hand their
  // FULL address to a board owner to be granted access (the abbreviated chip
  // alone isn't enough to grant). Direct listener → dies with the element on
  // remount, no accumulation.
  const abbr = addr => addr ? `${ addr.slice(0, 6) }…${ addr.slice(-4) }` : ''
  const abbrEl = document.getElementById('user-abbr')
  if (abbrEl) {
    abbrEl.innerHTML = `<button id="copy-my-address" class="copy-addr" title="Copy my full address (share it to be granted access)">${ abbr(activeAddress) }</button> <span class="role-tag role-${ role }">[${ role }]</span>`
    abbrEl.querySelector('#copy-my-address')?.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(activeAddress)
        notify('Address copied — send it to a board owner to receive access')
      } catch { notify('Could not copy the address', true) }
    })
  }
  ...
  // ---- Live edit capability. Two ORTHOGONAL permissions, mirroring what the
  // engine actually enforces per peer:
  //   • A shared board's ACL grant IS the authority for THAT node — the engine
  //     lets even a `guest` write it once granted (ACLs are opt-in per node and
  //     override the global role). So editing someone else's board depends only
  //     on ownership/grant, NOT on the RBAC role.
  //   • Only CREATING your own board (no node yet) needs the earned `user` role
  //     — that governance gate is what deters throwaway identities.
  let lastBoardValue = null
  const canEditBoard = () => {
    if (!lastBoardValue) return isMine && canWriteRBAC // creating my own board
  ...
  dragUpdate = updateProjectData
  if (!dragWired) {
    drag((...args) => dragUpdate(...args))
    dragWired = true
  }
  ...
  function buildRetrievedProject (data) {
    ...
    // No template re-injection here: it would paint a VIRGIN header (empty
    // address chip, no Share) over the mounted one. If the container is gone,
    // this callback belongs to a stale mount — drop the frame.
    ...
    const projectHTML = `${ projectData.data.map((col, i) =>
      `<div id="stage-${ col.colNumber }" class="col ${ col.tracking }-gradient ${ col.tracking }-border" data-track="${ col.tracking }" draggable="true">
        ...
        `<div id="card-${ card.cardNumber }" class="card" draggable="true" data-card-track="${ col.tracking }" data-card-md="${ card.cardText }" data-card-bg="${ card.cardBg || '' }"${ card.cardBg ? ` style="--card-neon:${ card.cardBg }"` : '' }>
  ...
  // Neon accent palette for cards (saturated → glows on the dark surface)
  const NEON = ['#00e5ff', '#39ff14', '#ff3d9a', '#ffe14d', '#b57bff', '#ff9e3d']

  function openAddCardForm (column) {
    const cardContainer = document.getElementById(`cards-${ column }`)
    const swatches = NEON.map(c => `<li class="color" data-color="${ c }" style="background:${ c }; box-shadow: 0 0 8px ${ c }"></li>`).join('')
    const addCardTemplate = `
      <div id="add-card-div-${ column }" class="add-card-box">
        <ul class="color-list">${ swatches }</ul>
        <form>
    ...
    // Pick a neon accent (optional): store it on the box, preview on the textarea
    const box = cardContainer.querySelector('.add-card-box')
    box?.querySelectorAll('.color').forEach(el => {
      el.onclick = () => {
        const c = el.getAttribute('data-color')
        box.setAttribute('data-bg', c)
        box.querySelectorAll('.color').forEach(s => s.classList.remove('active'))
        el.classList.add('active')
        const ta = box.querySelector('textarea')
        if (ta) { ta.style.borderLeft = `3px solid ${ c }`; ta.style.boxShadow = `-5px 0 16px -10px ${ c }` }
      }
    })
  }
  function addCard (column) {
    ...
    const cardTemplate = `
      <div id="card-${ newCardNo }" class="card" draggable="true" data-card-track="${ colTracking }" data-card-md="${ newNoteText }" data-card-bg="${ bgColor }"${ bgColor ? ` style="--card-neon:${ bgColor }"` : '' }>
