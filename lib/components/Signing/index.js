...
export default async function mountSigning ({ db, mount, onAuthenticated }) {
  ...
  const finishAuth = identity => {
    if (identity?.address) {
      const short = abbr(identity.address)
      alert('', '', `Welcome, ${ short }`, 'success', false, true)
    }
    if (typeof onAuthenticated === 'function') onAuthenticated(identity)
  }
  ...
  const cred = await navigator.credentials.create({ publicKey })
      ...
      // Persist the mnemonic locally bound to this credential id for auto-login
      if (candidate) {
        try {
          localStorage.setItem(`webauthn-mnemonic-${record.rawId}`, candidate)
          localStorage.setItem('webauthn-last', record.rawId)
        } catch (_) {}
      }
